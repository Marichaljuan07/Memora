...
/* ==========================================================================
   SISTEMA DE ACCESO LIBRE E INICIALIZACIÓN
   ========================================================================== */
...
function iniciarRelojHeader() {
    function actualizar() {
        ...
        if ($('relojHeader')) $('relojHeader').innerText = `${fechaFmt}   ${horaFmt}`;
    }
    actualizar();
...
function navegarA(pantalla, customTitle = null) {
    ...
    const titleMap = {
        'inicio': 'Inicio', 'registros': 'Registros',
        'formulario': editando ? 'Editar Cliente' : 'Nuevo Cliente',
        'perfil': 'Perfil', 'ficha': 'Ficha Cliente'
    };
...
function mostrarCanal() {
    let c = $('canal').value;
    let nombres = { 
        WhatsApp: 'Teléfono / WhatsApp', Instagram: 'Usuario Instagram (@usuario)', 
...
function agregarComentarioFormulario() {
    ...
    comentariosEdicionActual.push({
        texto: txt, fecha: fechaHoraTextoFormateada(), editado: null, eliminado: false
    });
    $('comentario').value = '';
...
function guardarDatosUsuarioAdmin() {
    ...
    if ($('sec-inicio').style.display === 'none' && $('sec-registros').style.display === 'none') navegarA('inicio');
}
...
window.addEventListener('online', () => { if ($('netStatus')) $('netStatus').innerHTML = '<span class="status-dot"></span><span class="status-text">Online</span>'; });
window.addEventListener('offline', () => { if ($('netStatus')) $('netStatus').innerHTML = '<span class="status-dot offline"></span><span class="status-text">Offline</span>'; });
