...
function actualizarSeguimiento() {
    ...
    if ($('textoBannerSeguimiento')) {
        $('textoBannerSeguimiento').textContent = textoIdiomaMemora150(
            'Memora vuelve a mostrarte los registros que superan el tiempo sin actividad que configuraste, para que revises qué pasó y decidas cómo continuar.',
            'Memora brings back records that stay inactive longer than the time you set, so you can review what happened and decide what to do next.',
            'O Memora volta a mostrar os registros que ficam sem atividade por mais tempo do que você definiu, para que você revise o que aconteceu e decida como continuar.'
        );
    }
    ...
    if ($('contenedorSeguimiento')) {
        ...
        $('contenedorSeguimiento').innerHTML = lista.map(r => {
            ...
            const comentariosActivosSeguimiento = (r.comentarios || []).filter(c => !c.eliminado && String(c.texto || '').trim());
            const ultimoComentarioSeguimiento = comentariosActivosSeguimiento.length
                ? String(comentariosActivosSeguimiento[comentariosActivosSeguimiento.length - 1].texto || '').trim()
                : '';
...
function obtenerTextoIdentificador(r) {
    ...
    const tipoVisible = typeof traducirCadenaMemora === 'function'
        ? traducirCadenaMemora(tipo)
        : tipo;
    return ` • ${tipoVisible}: ${limpio}`;
}
...
function agregarCampoCanalExtraInicio(canalVal = '', contactoVal = '') {
    ...
    div.innerHTML = `
        <button type="button" onclick="quitarCampoCanalExtraInicio(${idNum})" style="position:absolute; top:6px; right:8px; background:none; border:none; color:#DC2626; font-weight:700; font-size:0.75rem; cursor:pointer;">✖ ${traducirCadenaMemora('Quitar')}</button>
...
function agregarCampoCanalExtraMovil(canalVal = '', contactoVal = '') {
    ...
    div.innerHTML = `
        <button type="button" onclick="quitarCampoCanalExtraMovil(${idNum})" style="position:absolute; top:6px; right:8px; background:none; border:none; color:#DC2626; font-weight:700; font-size:0.75rem; cursor:pointer;">✖ ${traducirCadenaMemora('Quitar')}</button>
        <div style="display:grid; grid-template-columns: 1fr 2fr; gap:8px; margin-top:12px;">
            <div>
                <label style="font-size:0.75rem; font-weight:600; color:var(--text-secondary);">Canal Extra</label>
        ...
        <div class="card" style="padding:10px; margin-top:6px; font-size:0.8rem; background:#F9FAFB; border:1px solid #E5E7EB; border-radius:8px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:4px; font-size:0.7rem; color:var(--text-secondary);">
                ...
                <div>
                    ...
                    <a href="#" onclick="eliminarComentarioTemporalInicio(${i}); return false;" style="color:#DC2626; font-weight:600; text-decoration:none;">${traducirCadenaMemora('Eliminar')}</a>
                </div>` : ''}
            </div>
...
function guardarDesdeInicio() {
    ...
    if (editando && registroOriginal) {
        ...
        if (!hubocambios) {
            ...
            render();
            mostrarToastPC("Sin cambios registrados. Se conserva la fecha de la última revisión.", "info");
            return;
        }
...
function guardar() {
    ...
    if (editando && registroOriginal) {
        ...
        if (!hubocambios) {
            limpiar();
            navegarA('registros');
            // En móvil no interrumpimos con aviso: se conserva la última revisión y se vuelve al listado.
            return;
        }
        const ahoraISO = ahoraMemora().toISOString();
        rProvisorio.ultimaModificacion = ahoraISO;
...
function editar(id) {
    ...
    if (esPC) {
        ...
        if ($('formTitulo')) $('formTitulo').innerText = traducirCadenaMemora('Editar Registro');
...
/* ==========================================================================
   TRADUCCIÓN OPCIONAL DE TEXTO ESCRITO POR EL USUARIO
   ========================================================================== */
...
function abrirTraduccionTextoMemora(textoCodificado, event = null) {
    if (event) {
        event.preventDefault();
        event.stopPropagation();
    }

    const texto = decodeURIComponent(String(textoCodificado || ''));
    if (!texto.trim()) return;

    if (navigator.onLine === false) {
        mostrarAvisoMemora(
            traducirCadenaMemora('La traducción requiere conexión a Internet.'),
            traducirCadenaMemora('Traducir texto'),
            'language'
        );
        return;
    }

    // No modifica ni reemplaza el comentario original. Abre una traducción
    // externa hacia el idioma activo de Memora, con detección automática del origen.
    const destino = idiomaMemora();
    const url = `https://translate.google.com/?sl=auto&tl=${encodeURIComponent(destino)}&text=${encodeURIComponent(texto)}&op=translate`;
    window.open(url, '_blank', 'noopener,noreferrer');
}

function toggleAyudaMemora() {
    const contenido = $('contenidoAyudaMemora');
    const icono = $('iconoAyudaMemora');
    const boton = $('botonAyudaMemora');
    if (!contenido) return;

    const abrir = contenido.style.display === 'none' || !contenido.style.display;
    contenido.style.display = abrir ? 'block' : 'none';
    if (icono) icono.innerText = abrir ? 'expand_less' : 'expand_more';
...
function construirHojaXLSXMemora(datosAExportar, filtro, fechaExportacion = ahoraMemora()) {
    ...
    filasXML.push(
        `<row r="2" ht="20" customHeight="1">${celdaTextoXLSX('A2', 'Gestión y seguimiento de clientes', 2)}</row>`
    );
...
function construirPDFMemora(datosAExportar, filtro, fechaExportacion = ahoraMemora()) {
    ...
    const dibujarEncabezado = () => {
        ...
        comandos += comandoTextoPDFMemora(margenX, 748, 'Gestión y seguimiento de clientes', 9.5, false, '0.35 0.39 0.45');
...
function limpiar() {
    ...
    if ($('formTitulo')) {
        $('formTitulo').innerText = traducirCadenaMemora('Nuevo Registro');
    }
}
...
const MEMORA_TRADUCCIONES = {
    en: {
        ...
        'Conectado a Google Drive': 'Connected to Google Drive',
        'Conectar': 'Connect',
        ...
        'Registros para revisar': 'Records to review',
        'Traducir texto': 'Translate text',
        'La traducción requiere conexión a Internet.': 'Translation requires an Internet connection.',
    ...
    pt: {
        ...
        'Conectado a Google Drive': 'Conectado ao Google Drive',
        'Conectar': 'Conectar',
        ...
        'Ayuda': 'Ajuda',
        'Eliminación de datos': 'Exclusão de dados',
        'Registros para revisar': 'Registros para revisar',
        'Traducir texto': 'Traduzir texto',
        'La traducción requiere conexión a Internet.': 'A tradução requer conexão com a Internet.',
...
Object.assign(MEMORA_TRADUCCIONES.en, {
    ...
    'Definí cuánto tiempo puede pasar sin actividad. Memora vuelve a poner delante los registros que superan ese plazo para que revises qué pasó y decidas cómo continuar.': 'Set how long a record can remain inactive. Memora brings records that exceed that time back to your attention so you can review what happened and decide how to continue.',
    ...
    'Gestión y seguimiento de clientes': 'Customer management and follow-up',
    ...
    'Sin cambios registrados. Se conserva la fecha de la última revisión.': 'No changes recorded. The last review date is preserved.',
...
Object.assign(MEMORA_TRADUCCIONES.pt, {
    ...
    'Definí cuánto tiempo puede pasar sin actividad. Memora vuelve a poner delante los registros que superan ese plazo para que revises qué pasó y decidas cómo continuar.': 'Defina quanto tempo um registro pode ficar sem atividade. O Memora volta a colocar em destaque os registros que ultrapassam esse prazo para você revisar o que aconteceu e decidir como continuar.',
    ...
    'Gestión y seguimiento de clientes': 'Gestão e acompanhamento de clientes',
    ...
    'Sin cambios registrados. Se conserva la fecha de la última revisión.': 'Nenhuma alteração registrada. A data da última revisão é mantida.',
...
construirHojaXLSXMemora = function(datosAExportar, filtro, fechaExportacion = ahoraMemora()) {
    ...
    filasXML.push(`<row r="2" ht="20" customHeight="1">${celdaTextoXLSX('A2', textoIdiomaMemora150('Gestión y seguimiento de clientes','Customer management and follow-up','Gestão e acompanhamento de clientes'), 2)}</row>`);
    filasXML.push(`<row r="3" ht="8" customHeight="1"></row>`);
...
construirPDFMemora = function(datosAExportar, filtro, fechaExportacion = ahoraMemora()) {
    ...
    const dibujarEncabezado=()=>{
        ...
        comandos+=comandoTextoPDFMemora(margenX,748,textoIdiomaMemora150('Gestión y seguimiento de clientes','Customer management and follow-up','Gestão e acompanhamento de clientes'),9.5,false,'0.35 0.39 0.45');
