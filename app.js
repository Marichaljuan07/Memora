...
/* ==========================================================================
   SISTEMA DE ACTIVACIÓN POR CLAVE + CONFIGURACIÓN INICIAL DE PERFIL
   ========================================================================== */
function comprobarEstadoAccesoEInicial() {
    ...
    if (!perfilCompleto) {
        abrirModalConfiguracionInicialPerfil();
        return;
    }
...
function pedirClaveModal() {
    const claveIngresada = prompt("🔑 ¡Bienvenido a MEMORA!\n\nIngresa tu clave de acceso para activar tu licencia:");
    if (!claveIngresada) {
        alert("Debes ingresar una clave para usar la aplicación.");
        pedirClaveModal();
        return;
    }

    const claveLimpia = claveIngresada.trim().toUpperCase();
    const ahora = new Date().getTime();

    if (claveLimpia === "JUAN-CREATOR-2026") {
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_user_role', 'creator');
        alert("¡Bienvenido Creador! Licencia activada.");
        comprobarEstadoAccesoEInicial();
        return;
    }

    if (claveLimpia.startsWith("MEM-") || claveLimpia === "MEMORA2026") {
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_user_role', 'customer');
        alert("¡Clave aceptada! Licencia de por vida activada.");
        comprobarEstadoAccesoEInicial();
        return;
    }

    if (claveLimpia === "PRUEBA-1DIA") {
        const expira = ahora + (24 * 60 * 60 * 1000);
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_trial_expires', expira);
        localStorage.setItem('memora_user_role', 'guest');
        alert("¡Prueba de 24 horas activada!");
        comprobarEstadoAccesoEInicial();
        return;
    }

    alert("❌ Clave no válida. Por favor, verifica el código ingresado.");
    pedirClaveModal();
}

function abrirModalConfiguracionInicialPerfil() {
    const modal = $('modalConfigAdmin');
    if (modal) {
        modal.style.display = 'block';
        navegarA('perfil', 'Configuración Inicial');
        alert("👤 ¡Licencia activada con éxito!\n\nPor favor, completa tus datos personales y de empresa por única vez para personalizar tu perfil y bienvenida.");
    }
}
...
/* GUARDAR PERFIL Y LIGAR NOMBRE AL SALUDO DE INICIO */
function guardarDatosUsuarioAdmin() {
    ...
    // Si estaba configurando por primera vez, redirigir al Inicio
    if ($('sec-inicio').style.display === 'none' && $('sec-registros').style.display === 'none') {
        navegarA('inicio');
    }
}

/* CARGAR PERFIL Y ACTUALIZAR SALUDO EN INICIO */
...
/* ==========================================================================
   DIAGNÓSTICO TÉCNICO COMPLETO (DISPOSITIVO, NAVEGADOR Y DIRECCIÓN IP)
   ========================================================================== */
/* ==========================================================================
   DIAGNÓSTICO TÉCNICO COMPLETO (DETECCIÓN EXACTA DE BRAVE + IP)
   ========================================================================== */
async function cargarDiagnosticoSistema() {
    const ua = navigator.userAgent;

    // 1. Detección precisa de Dispositivo y Sistema Operativo
    ...
    // 2. Detección precisa de Navegador (Incluye comprobación explícita de Brave)
    let nav = "Navegador Web";
    
    // Comprobación especial para Brave
    ...
    // 3. Cálculo de Almacenamiento Local
    ...
    // 4. Obtención opcional de IP
    try {
        const ipResp = await fetch('https://api.ipify.org?format=json');
        if (ipResp.ok) {
            const ipData = await ipResp.json();
            if ($('sys-ip')) $('sys-ip').innerText = ipData.ip;
        }
    } catch (err) {
        if ($('sys-ip')) $('sys-ip').innerText = "No disponible";
    }
}
...
/* ==========================================================================
   FLUIJO DE BLOQUEO, ACTIVACIÓN Y STORIES DE BIENVENIDA
   ========================================================================== */
...
function comprobarEstadoAccesoEInicial() {
    ...
    // 1. Ocultar contenido principal hasta completar los pasos
    document.querySelector('.main-content').style.filter = 'blur(8px)';
    document.querySelector('.bottom-nav').style.display = 'none';

    if (!activado) {
        $('modalClaveMemora').style.display = 'flex';
        return;
    }
    ...
    // Si ya completó todo, liberar interfaz
...
function procesarClaveAcceso() {
    ...
    if (claveLimpia === "JUAN-CREATOR-2026") {
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_user_role', 'creator');
    } else if (claveLimpia.startsWith("MEM-") || claveLimpia === "MEMORA2026") {
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_user_role', 'customer');
    } else if (claveLimpia === "PRUEBA-1DIA") {
        const expira = ahora + (24 * 60 * 60 * 1000);
        localStorage.setItem('memora_activated', 'true');
        localStorage.setItem('memora_trial_expires', expira);
        localStorage.setItem('memora_user_role', 'guest');
    } else {
        alert("❌ Clave no válida. Revisa el código ingresado.");
        return;
    }
